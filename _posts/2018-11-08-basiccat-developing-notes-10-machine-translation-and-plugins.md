...
另外，我在开发亚马逊机器翻译插件时，发现它的签名算法过于复杂，于是直接使用了它提供的java sdk。但这带来的问题是需要在主程序里包含需要引用的第三方jar。aws translate使用的jar加起来有4MB。插件的话最好是在不改变主程序的情况下就能使用。我采取的解决办法是这样的：写一个命令行工具来调用亚马逊的机器翻译，然后使用插件来运行这个工具，把结果传回主程序。
